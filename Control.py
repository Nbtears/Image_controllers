...
def image_process(frame, mp_drawing, mp_holistic, holistic, control):
    ...
    try:
        ...
        if arm[0] == 'R':
            ...
            # look angle
            cv.putText(image, str(int(angle)), tuple(np.multiply(C, [647, 510]).astype(
                int)), cv.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2, cv.LINE_AA)
    ...
    if c == True:
        if lm_lh is not None:
            coor[len(
                coor)-1, 0] = int(lm_lh.landmark[mp_holistic.HandLandmark.WRIST].x*width)
            coor[len(
                coor)-1, 1] = int(lm_lh.landmark[mp_holistic.HandLandmark.WRIST].y*height)
        ...
        if lm_rh is not None:
            coor[0, 0] = int(
                lm_rh.landmark[mp_holistic.HandLandmark.WRIST].x*width)
            coor[0, 1] = int(
                lm_rh.landmark[mp_holistic.HandLandmark.WRIST].y*height)

            mp_drawing.draw_landmarks(image, lm_rh, mp_holistic.HAND_CONNECTIONS,
                                      mp_drawing.DrawingSpec(
                                          color=(102, 31, 208), thickness=2, circle_radius=3),
                                      mp_drawing.DrawingSpec(color=(219, 230, 101), thickness=2, circle_radius=2))
        ...
        cv.putText(image, "V. Angular = {:.2f}".format(
            v), (10, 30), cv.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0), 2, cv.LINE_AA)
        cv.putText(image, "A. Angular = {:.2f}".format(
            w), (410, 30), cv.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0), 2, cv.LINE_AA)
...
def Imagen():
    ...
    with mp_holistic.Holistic(min_detection_confidence=0.8, min_tracking_confidence=0.8)as holistic:
        while True:
            ...
            image, control = image_process(
                frame, mp_drawing, mp_holistic, holistic, control)
